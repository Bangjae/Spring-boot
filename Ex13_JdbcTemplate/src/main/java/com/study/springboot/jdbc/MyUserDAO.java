...
// @Repositoty 어노테이션을 통해 스프링 컨테이너 시작 시 자동으로 빈이 생성된다.
@Repository // 컨포넌트
public class MyUserDAO
{
	...
	@Autowired
	//자동으로 오라클 안의 객체를 연결
	// apllication.properties의 오라클 정보를 사용
	...
	public List<MyUserDTO> listForBeanPropertyRowMapper()
	{
		...
		/*
			2개 이상의 레코드를 인출하는 select 쿼리문을 실행할 때
			query메서드를 호출한다. 두번째 인수인 RowMapper가 인출된 레코드를
			DTO에 저장한 후 list에 추가하여 반환하게 된다.
