...
/*
 	Spring Lagacy에서 XML 설정파일로 만들었던 것을 Boot에서는 XML을 사용하지 않으므로,
 	아래와 같이 Java파일로 만든다.
 	이 때, 설정파일의 역할을 부여하기 위해 @Configuration 어노테이션을 부착한다.
...
public class Config
{
	...
	// 어플리케이션 컨텍스트(ApplicateionContext) : 빈 팩토리를 확장한 IoC 컨테이너

	
	/*
		@Bean 어노테이션을 통해 자바빈을 생성한다.
		이 때, 참조변수명은 별도의 설정이 없으므로 메서드명인 member1로 생성된다.
	*/
	@Bean
